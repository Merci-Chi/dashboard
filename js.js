(() => {
  ...
  const ALL_VIEWS = [
    "dashboard", "leads", "clients", "tasks", "ideas", "scripts", "assets", "seo",
    "domain", "hosting", "prospects", "onboarding", "data-collection", "payment",
   ...
   * Any Supabase user not listed here gets UNAUTHORIZED.
   * ============================================================
   */
  const USER_ACCESS = {
  ...
  function getDisplayName(currentSession = session) {
    ...
    const name = meta.display_name || meta.full_name || meta.name || meta.user_name || meta.username;
  ...
  function needsFirstSetup(currentSession = session) {
    ...
    const existingName = meta.display_name || meta.full_name || meta.name || meta.user_name || meta.username;
  ...
  function prefillSetup() {
    ...
    input.value = String(meta.display_name || meta.full_name || meta.name || meta.user_name || meta.username || "").trim();
  }

  function showView(requestedView) {
    let view = requestedView;
    if (!allowedViews.includes(view)) view = allowedViews[0] || "unauthorized";
  ...
  function applyPermissions() {
    ...
    $$('[data-view]').forEach(item => {
      item.classList.toggle('permission-hidden', !allowedViews.includes(item.dataset.view));
    });
    $$('.nav-section').forEach(section => {
      const visible = [...section.querySelectorAll('[data-view]')].some(item => !item.classList.contains('permission-hidden'));
      section.classList.toggle('permission-hidden', !visible);
    });
  ...
  function reloadLeadsAfterLogin() {
    ...
    const deliverSession = () => {
      sendSessionToLeadsFrame();
      setTimeout(sendSessionToLeadsFrame, 250);
    };

    frame.onload = deliverSession;

    if (leadsLoadedForUser !== userId) {
      ...
      frame.src = `${baseSrc.split('?')[0]}?session=${Date.now()}`;
    } else {
      deliverSession();
    }
  }
  ...
  function routeSession() {
    ...
    setScreen('app');
    applyPermissions();
    if (allowedViews.includes('leads')) reloadLeadsAfterLogin();
  }
  ...
  function setupNavigation() {
    $$('[data-view]').forEach(item => {
      item.addEventListener('click', event => {
        ...
        if (allowedViews.includes(item.dataset.view)) showView(item.dataset.view);
      });
    });
    window.addEventListener('hashchange', () => {
      if (!session?.user) return;
      const requested = location.hash.slice(1);
      if (allowedViews.includes(requested)) showView(requested);
    });
  }
