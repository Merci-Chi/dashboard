...
async function listBilling(){
  ...
  const {data,error}=await client.from('square')
    .select('*, payments(*)')
    .in('status',['ACTIVE','CANCELED','PAUSED','DEACTIVATED','PENDING'])
...
const root=document.getElementById('clientList'),status=document.getElementById('status'),esc=escapeHTML;let clients=[],saveTimers=new Map();const unlocked=new Set();
