(() => {
  ...
  const rules = [
    ...
    [12,/floor|flooring|tile install/],[13,/fenc|fencing/],[14,/junk|hauling|haul away|debris removal/],[15,/pest|exterminat|termite/],[16,/cleaning|cleaners|maid|janitorial|housekeeping/],
  ...
  function classification(lead) {
    ...
    const text = [rawType,lead.company,lead.name,lead.issue,lead.notes,lead.concerns].filter(Boolean).join(' ').toLowerCase().replace(/[_-]+/g,' ');
    for (const [rank,pattern] of rules) if (pattern.test(text)) return byRank(rank);
  ...
  function leadPreview(lead) {
    return previewUrl(lead.previewurl, previewByLead.get(String(lead.id)));
  }
  ...
  function sortedRows() {
    ...
    return crmRows
      ...
      .filter(lead => !q || [lead.company,lead.name,lead.phone,lead.email,lead.leadtype].some(value => String(value || '').toLowerCase().includes(q)))
      .slice()
