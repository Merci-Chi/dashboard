(() => {
  ...
  async function getAuthor(){
    const {data}=await client.auth.getUser();const u=data?.user;if(!u)return null;
  ...
  function render(){
    ...
    list.innerHTML=rows.map(r=>`<article class="na-card ${r.kind==='alert'?'alert':''}"><div class="na-icon"><i class="bi ${esc(r.icon|| (r.kind==='alert'?'bi-bell-fill':'bi-journal-text'))}"></i></div><div><h4>${esc(r.title)}</h4>${r.description?`<p class="na-desc">${esc(r.description)}</p>`:''}<div class="na-meta">${esc(r.author_name||'Team member')} · ${new Date(r.created_at).toLocaleString()}</div></div></article>`).join('');
  }
