...
[BUSINESS RESEARCH REPORT from step one HERE]`;
 const hasCrmValue=value=>{if(value===null||value===undefined)return false;if(typeof value==='string'){const text=value.trim().toLowerCase();return text!==''&&text!=='null'&&text!=='undefined'}if(Array.isArray(value))return value.some(hasCrmValue);if(typeof value==='object')return Object.values(value).some(hasCrmValue);return true};
 const crmLabel=key=>String(key).replace(/_/g,' ').replace(/\b\w/g,letter=>letter.toUpperCase());
 const crmValue=value=>{if(typeof value==='string')return value.trim();if(typeof value==='object')return JSON.stringify(value,null,2);return String(value)};
 function crmResearchContext(lead){const fields=Object.entries(lead||{}).filter(([,value])=>hasCrmValue(value));if(!fields.length)return'';return `\n\n**Existing CRM Information**\n\nUse all relevant information below as research context. Verify public-facing facts with reliable public sources when possible. Do not publish private or internal information merely because it appears in the CRM.\n\n${fields.map(([key,value])=>`- **${crmLabel(key)}:** ${crmValue(value)}`).join('\n')}`}
 function workflowMarkup(){const company=active?.company||active?.name||'this company',fullResearchPrompt=researchPrompt.replace('[COMPANY NAME]',company)+crmResearchContext(active),developmentPrompt=websitePrompt.replace('[BUSINESS RESEARCH REPORT from step one HERE]',researchDraft||'[BUSINESS RESEARCH REPORT from step one HERE]');return `<section class="research-workflow"><div class="workflow-steps"><span class="workflow-step ${workflowStep===1?'active':'complete'}" data-workflow-dot="1">1</span><i></i><span class="workflow-step ${workflowStep===2?'active':''}" data-workflow-dot="2">2</span></div><div class="workflow-page ${workflowStep===1?'active':''}" data-workflow-page="1"><span class="workflow-kicker">STEP 1 · RESEARCH</span><h3>Research ${SitePipeline.escapeHTML(company)}</h3><p>Copy this prompt, complete the research, then paste the report below. All non-empty CRM information is included automatically.</p><textarea class="workflow-prompt" id="researchPrompt" readonly>${SitePipeline.escapeHTML(fullResearchPrompt)}</textarea><div class="workflow-actions"><button class="btn primary" data-copy-research type="button"><i class="bi bi-copy"></i> Copy Research Prompt</button></div><label class="workflow-label" for="researchResults">Paste research results</label><textarea class="workflow-results" id="researchResults" placeholder="Paste the completed Business Research Report here…">${SitePipeline.escapeHTML(researchDraft)}</textarea><div class="workflow-actions"><button class="btn primary" data-next-workflow type="button">Continue to Site Development <i class="bi bi-arrow-right"></i></button></div></div><div class="workflow-page ${workflowStep===2?'active':''}" data-workflow-page="2"><span class="workflow-kicker">STEP 2 · SITE DEVELOPMENT</span><h3>Create the website</h3><p>Your research report is already included at the bottom. Copy the complete prompt to begin the build.</p><textarea class="workflow-prompt" id="websitePrompt" readonly>${SitePipeline.escapeHTML(developmentPrompt)}</textarea><div class="workflow-actions"><button class="btn" data-back-workflow type="button"><i class="bi bi-arrow-left"></i> Back</button><button class="btn primary" data-copy-website type="button"><i class="bi bi-copy"></i> Copy Site Development Prompt</button></div></div></section>`}
 ...
 function confirmCopy(button,label){const original=button.innerHTML;button.classList.add('copy-confirmed');button.innerHTML=`<i class="bi bi-check2"></i> ${label}`;setTimeout(()=>{if(button.isConnected){button.classList.remove('copy-confirmed');button.innerHTML=original}},1400)}
 ...
 document.getElementById('uploadWorkspace').addEventListener('click',async e=>{const b=e.target.closest('button');if(!b)return;if(b.hasAttribute('data-copy-research')){await navigator.clipboard.writeText(document.getElementById('researchPrompt').value);confirmCopy(b,'Copied!');message('Research prompt copied.')}if(b.hasAttribute('data-copy-website')){await navigator.clipboard.writeText(document.getElementById('websitePrompt').value);confirmCopy(b,'Copied!');message('Site development prompt copied.')}if(b.hasAttribute('data-next-workflow')){const results=document.getElementById('researchResults');researchDraft=results.value.trim();if(!researchDraft)return message('Paste the research results before continuing.',true);document.getElementById('websitePrompt').value=websitePrompt.replace('[BUSINESS RESEARCH REPORT from step one HERE]',researchDraft);setWorkflowStep(2);window.scrollTo({top:0,behavior:'smooth'})}if(b.hasAttribute('data-back-workflow')){researchDraft=document.getElementById('researchResults')?.value.trim()||researchDraft;setWorkflowStep(1);window.scrollTo({top:0,behavior:'smooth'})}});
