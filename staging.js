...
Keep the report concise but thorough. Use short bullets, plain language, consistent capitalization, and Markdown tables exactly where specified. Every factual statement must include a source or clearly say “Not found,” “Unverified,” or “Suggested.”`;
 const websitePrompt=company=>`Create a polished, professional, mobile-responsive website for **${company}** using the verified research pasted below. Use only facts supported by the research; never invent missing details. If information conflicts, mark it for review instead of choosing silently.

Build a clear site structure with an engaging homepage, services/products, service areas, about/team, reviews, contact, hours, FAQ, and any other sections supported by the research. Use the company’s verified brand tone, colors, imagery links, differentiators, and calls to action. Make phone, email, map, social, and booking links functional when provided. Keep private addresses and unsupported claims out of the website.

Return:
1. A concise content plan and page structure.
2. Complete draft copy for every page and section.
3. A visual direction describing layout, typography, colors, spacing, and image usage.
4. SEO title, meta description, headings, and local SEO recommendations.
5. A list of facts or assets still needing confirmation.

Research results:

[PASTE RESEARCH RESULTS HERE]`;
 function workflowMarkup(){const company=active?.company||active?.name||'this company';return `<section class="research-workflow"><div class="workflow-steps"><span class="workflow-step active" data-workflow-dot="1">1</span><i></i><span class="workflow-step" data-workflow-dot="2">2</span></div><div class="workflow-page active" data-workflow-page="1"><span class="workflow-kicker">STEP 1 · RESEARCH</span><h3>Research ${SitePipeline.escapeHTML(company)}</h3><p>Copy this prompt, run your research, then paste the completed report below.</p><textarea class="workflow-prompt" id="researchPrompt" readonly>${SitePipeline.escapeHTML(researchPrompt.replace('[COMPANY NAME]',company))}</textarea><div class="workflow-actions"><button class="btn primary" data-copy-research type="button"><i class="bi bi-copy"></i> Copy Research Prompt</button></div><label class="workflow-label" for="researchResults">Paste research results</label><textarea class="workflow-results" id="researchResults" placeholder="Paste the completed Business Research Report here…"></textarea><div class="workflow-actions"><button class="btn primary" data-next-workflow type="button">Continue to Website Prompt <i class="bi bi-arrow-right"></i></button></div></div><div class="workflow-page" data-workflow-page="2"><span class="workflow-kicker">STEP 2 · WEBSITE</span><h3>Create the website</h3><p>Copy this prompt with the research results included.</p><textarea class="workflow-prompt" id="websitePrompt" readonly>${SitePipeline.escapeHTML(websitePrompt(company))}</textarea><div class="workflow-actions"><button class="btn" data-back-workflow type="button"><i class="bi bi-arrow-left"></i> Back</button><button class="btn primary" data-copy-website type="button"><i class="bi bi-copy"></i> Copy Website Prompt</button></div></div></section>`}
 function setWorkflowStep(step){document.querySelectorAll('[data-workflow-page]').forEach(page=>page.classList.toggle('active',Number(page.dataset.workflowPage)===step));document.querySelectorAll('[data-workflow-dot]').forEach(dot=>{const n=Number(dot.dataset.workflowDot);dot.classList.toggle('active',n===step);dot.classList.toggle('complete',n<step)})}
 function injectWorkflow(){const el=document.getElementById('uploadWorkspace');if(el&&!el.querySelector('.research-workflow')&&active)el.insertAdjacentHTML('afterbegin',workflowMarkup())}
 const workflowObserver=new MutationObserver(()=>injectWorkflow());workflowObserver.observe(document.getElementById('uploadWorkspace'),{childList:true});
 ...
 document.getElementById('uploadWorkspace').addEventListener('click',async e=>{const b=e.target.closest('button');if(!b)return;if(b.hasAttribute('data-copy-research')){await navigator.clipboard.writeText(document.getElementById('researchPrompt').value);message('Research prompt copied.')}if(b.hasAttribute('data-copy-website')){await navigator.clipboard.writeText(document.getElementById('websitePrompt').value);message('Website prompt copied.')}if(b.hasAttribute('data-next-workflow')){const results=document.getElementById('researchResults');if(!results.value.trim())return message('Paste the research results before continuing.',true);document.getElementById('websitePrompt').value=websitePrompt(active.company||active.name||'this company').replace('[PASTE RESEARCH RESULTS HERE]',results.value);setWorkflowStep(2)}if(b.hasAttribute('data-back-workflow'))setWorkflowStep(1)});
})();
