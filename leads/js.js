...
function supabaseRowToLead(row, status = 'new') {
  return ensureAutomaticTags({
    ...
    businessRank: Number(row.businessrank) || 999,
...
async function hydrateFromSupabase() {
  ...
  for (let from = 0; ; from += pageSize) {
    const result = await supabaseClient
      .from('crm')
      .select('*')
      .order('businessrank', { ascending: true })
  ...
  const loadedLeads = rows.map(row => supabaseRowToLead(row, crmStatus(row)));
...
function leadFirstNameForScript(lead) {
  const fullName = String(lead?.name || '').trim();
  if (!fullName) return '____';
  const first = fullName.split(/\s+/)[0].trim();
  return first || '____';
}

function updateQuickInfoScriptNames(lead = currentLead()) {
  const leadFirstName = leadFirstNameForScript(lead);
  const userName = activeUserName();

  document.querySelectorAll('.script-lead-first-name').forEach(el => {
    el.textContent = leadFirstName;
  });
...
function preferredContactMethods(lead) {
  const raw = lead?.preferredContact;
...
function normalizedContactMethodLabel(value) {
  const key = String(value || '').trim().toLowerCase();
  if (key === 'phone') return 'call';
  return key;
}

function updateContactPreferenceButtons(lead = currentLead()) {
  if (!lead) return;

  const selected = new Set(
    preferredContactMethods(lead).map(normalizedContactMethodLabel)
  );

  document.querySelectorAll('[data-contact-method]').forEach(button => {
    const value = normalizedContactMethodLabel(button.dataset.contactMethod);
...
function togglePreferredContactMethod(value) {
  const lead = currentLead();
  if (!lead) return;

  const method = normalizedContactMethodLabel(value);
  if (!method) return;
  ...
  const order = ['call', 'email', 'text', 'zoom'];
  lead.preferredContact = order
    .filter(item => selected.has(item))
    .map(item => item === 'call' ? 'Call' : item.charAt(0).toUpperCase() + item.slice(1))
    .join(', ');

  lead.updatedAt = new Date().toISOString();

  if (typeof saveState === 'function') saveState(lead.id);
  updateContactPreferenceButtons(lead);
}

function updatePreferredContactInfoFields(lead = currentLead()) {
  if (!lead) return;

  const phoneInput = document.getElementById('preferredPhone');
  const emailInput = document.getElementById('preferredEmail');

  if (phoneInput) phoneInput.value = String(lead.phone || '').trim();
  if (emailInput) emailInput.value = String(lead.email || '').trim();
}

function savePreferredPhoneField() {
  const lead = currentLead();
  const input = document.getElementById('preferredPhone');
  if (!lead || !input) return;

  lead.phone = String(input.value || '').trim();
  lead.updatedAt = new Date().toISOString();
...
function savePreferredEmailField() {
  const lead = currentLead();
  const input = document.getElementById('preferredEmail');
  if (!lead || !input) return;

  lead.email = String(input.value || '').trim();
  lead.updatedAt = new Date().toISOString();
...
function renderLeadHistory() {
  ...
  list.innerHTML = items.map(item => {
    ...
    const deleteButton = canDeleteHistory
      ? `<button class="history-note-delete" type="button" data-delete-history="${escapeHTML(item.id || '')}" aria-label="Delete history entry" title="Delete history entry"><i class="bi bi-x-lg"></i></button>`
      : '';
    return `
...
async function initializeSupabaseSession() {
  try {
    ...
    if (!supabaseSession) {
      // Access is controlled only by the parent/main app.
      // If this page is ever opened directly without a session,
      // simply leave the workspace without remote data.
...
function normalizeLeadType(value) {
  ...
  if (/site removed|website removed|removed site|removed website|site deleted|website deleted/.test(key)) return 'Site Removed';
...
const LEAD_TAG_ICONS = {
  ...
  'Site Removed': 'bi-file-earmark-x',
...
function ensureNoSiteTag(lead) {
  ...
  // Website tags are intentionally multi-select. Keep every selected website tag.
  const normalizedTags = [];
  const seen = new Set();
  lead.tags.forEach(tag => {
    const normalized = normalizeLeadType(tag);
...
function ensureNoPhoneTag(lead) {
  if (!lead) return lead;
  lead.tags = Array.isArray(lead.tags) ? lead.tags : [];
  // Manual toggle: never force this tag back on or off automatically.
  return lead;
}
  ...
  const groupHtml = LEAD_TAG_GROUPS.map(group => {
    ...
    return `
      <section class="lead-tag-group lead-tag-group-${escapeHTML(group.key)}">
        ...
        <div class="lead-tag-group-options" ${group.key === 'site' ? 'aria-label="Website tags"' : ''}>
          ${chips}
        </div>
      </section>`;
  }).join('');
...
async function toggleQuickInfoTag(label) {
  ...
  if (normalized === 'Spanish?') {
    ...
    // Website tags are true independent toggles. Multiple may be active together.
    if (isSelected) {
      removeMatchingTag();

      // If this tag was the compatibility/default leadType, fall back to another
      // selected website tag instead of forcing the removed tag back on.
      if (normalizeLeadType(lead.leadType) === normalized) {
        lead.leadType = lead.tags
          .map(tag => normalizeLeadType(tag))
          .find(tag => LEAD_TYPE_TAGS.has(tag)) || '';
      }
    } else {
      addTagIfMissing();

      // Preserve a useful primary site condition for call-script compatibility,
      // but never use it to deselect any other tag.
      if (!LEAD_TYPE_TAGS.has(normalizeLeadType(lead.leadType))) {
...
function leadCard(lead) {
  ...
  const selectedSiteTags = Array.from(new Set([
    ...(Array.isArray(lead.tags) ? lead.tags.map(tag => normalizeLeadType(tag)).filter(tag => LEAD_TYPE_TAGS.has(tag)) : []),
    ...(LEAD_TYPE_TAGS.has(normalizeLeadType(lead.leadType)) ? [normalizeLeadType(lead.leadType)] : [])
  ]));
  const rawLeadTags = Array.isArray(lead.tags)
    ? lead.tags.map(tag => String(tag || '').trim()).filter(Boolean)
    : [];

  const normalizedSiteTagKeys = new Set(
    selectedSiteTags.map(tag => String(tag || '').trim().toLowerCase())
  );

  const hiddenCardTagKeys = new Set([
    'sold',
    'conversion'
  ]);

  const extraCardTags = Array.from(new Set(
    rawLeadTags.filter(tag => {
      const key = tag.toLowerCase();
      if (!key) return false;
      if (normalizedSiteTagKeys.has(key)) return false;
      if (hiddenCardTagKeys.has(key)) return false;
      return true;
    })
  ));
    ...
    <div class="lead-item-wrap${cornerTags ? ' has-priority-tags' : ''}${isHot ? ' has-hot-priority' : ''}${isWrongNumber ? ' has-wrong-priority' : ''}">
      ...
      <button class="lead-item desktop-lead-card${isHot ? ' hot-lead-card' : ''}${isWrongNumber ? ' wrong-number-card' : ''}" type="button" data-open-lead="${lead.id}">
        ...
        <span class="lead-copy lead-card-copy-ordered">
          <span class="lead-card-company">${escapeHTML(lead.company || 'No company')}</span>
          <span class="lead-card-contact">${escapeHTML(lead.name || 'No contact name')}</span>
          ${isSold
            ? `<span class="lead-card-call-status sold-by-card"><i class="bi bi-trophy-fill" aria-hidden="true"></i>Sold by ${escapeHTML(lead.soldBy || latestSoldActor(lead) || 'Unassigned')}</span>`
            : `<span class="lead-card-call-status ${latestCallHistory(lead) ? 'has-call' : 'no-call'}"><i class="bi bi-telephone-fill" aria-hidden="true"></i>${escapeHTML(callerSummary(lead, false))}</span>`}
          <span class="lead-card-reference">${sourceBadges || '<span class="lead-reference-empty">No reference</span>'}</span>
        </span>
...
function renderCurrentLead() {
  ...
  updateQuickInfoScriptNames(lead);
  updateContactPreferenceButtons(lead);
  ...
  if ($('#callbackEditor') && $('#addCallbackButton')) {
  ...
  const selectedCallOutcomes = new Set(
    (Array.isArray(lead.tags) ? lead.tags : [])
      .map(tag => normalizeCallOutcome(tag))
      .filter(tag => CALL_OUTCOME_VALUES.has(tag))
  );
  const scalarOutcome = normalizeCallOutcome(lead.outcome || lead.tag || '');
  if (CALL_OUTCOME_VALUES.has(scalarOutcome)) selectedCallOutcomes.add(scalarOutcome);
...
const CALL_OUTCOME_VALUES = new Set([
  'Interested',
  'Call Back',
  'Needs More Info',
  'Skeptical',
...
async function setFollowupCallOutcome(value) {
  ...
  if (!CALL_OUTCOME_VALUES.has(normalizedValue)) return;

  lead.tags = Array.isArray(lead.tags) ? lead.tags : [];
  const key = normalizedValue.toLowerCase();
  const isSelected = lead.tags.some(tag => normalizeCallOutcome(tag).toLowerCase() === key);

  if (isSelected) {
    lead.tags = lead.tags.filter(tag => normalizeCallOutcome(tag).toLowerCase() !== key);
    if (normalizeCallOutcome(lead.outcome || '').toLowerCase() === key) {
      lead.outcome = lead.tags.map(tag => normalizeCallOutcome(tag)).find(tag => CALL_OUTCOME_VALUES.has(tag)) || '';
    }
    if (normalizeCallOutcome(lead.tag || '').toLowerCase() === key) {
      lead.tag = lead.tags.map(tag => normalizeCallOutcome(tag)).find(tag => CALL_OUTCOME_VALUES.has(tag)) || '';
    }
  } else {
    lead.tags.push(normalizedValue);
    lead.outcome = normalizedValue;
    lead.tag = normalizedValue;
  }
  ...
  try {
    ...
    console.error('Could not sync call outcomes:', error);
    queueLeadSync(lead.id);
  }
}
...
supabaseClient.auth.onAuthStateChange(async (_event, nextSession) => {
  ...
  supabaseSession = nextSession || null;
  ...
  if (previousUserId !== nextUserId || !previousUserId) {
    try {
      ...
      console.error('Lead sync after session change failed:', error);
      showSyncStatus('Sync failed');
    }
    return;
  }
...
/* CONTACT PREFERENCE — MULTI-SELECT TOGGLES */
document.querySelectorAll('[data-contact-method]').forEach(button => {
  button.addEventListener('click', event => {
    event.preventDefault();
    togglePreferredContactMethod(button.dataset.contactMethod);
  });
});

/* CONTACT INFO — PHONE + EMAIL */
const preferredPhoneInput = document.getElementById('preferredPhone');
if (preferredPhoneInput) {
  preferredPhoneInput.addEventListener('blur', savePreferredPhoneField);
  preferredPhoneInput.addEventListener('change', savePreferredPhoneField);
  preferredPhoneInput.addEventListener('keydown', event => {
    if (event.key === 'Enter') {
      event.preventDefault();
      preferredPhoneInput.blur();
    }
  });
}

const preferredEmailInput = document.getElementById('preferredEmail');
if (preferredEmailInput) {
  preferredEmailInput.addEventListener('blur', savePreferredEmailField);
  preferredEmailInput.addEventListener('change', savePreferredEmailField);
  preferredEmailInput.addEventListener('keydown', event => {
    if (event.key === 'Enter') {
      event.preventDefault();
